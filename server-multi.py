...
def handle_client(conn, addr): #Función para manejo del cliente
    ...
    while connected: #Ciclo while que se ejecuta mientras la variable 'connected' sea verdadera
        ...
        if msg == DISCONNECT_MSG: #Condicional: si el mensaje es desconectado entonces la variable connected se torna falsa permitiendo salir del ciclo while
            connected = False

        print(f"[{addr}] {msg}") #Con el uso de formatted string literal (f) imprime en consola la dirección y el mensaje
        conn.send(msg.encode(FORMAT)) #Con el uso del objeto 'conn' se envía el mensaje codificado
